"""utils 工具模块（从参考项目移植，import 路径已适配 cd_monitor）。

save_to_jsonl 这里改为纯 JSONL 落盘（不依赖 result_storage_service，
因为结果记录统一走 cd_monitor 的 result_items 表，调用方应直接用 repo）。
"""
            ...
            for i in range(retries):
                try:
                    ...
                    if hasattr(e, "response") and hasattr(e.response, "text"):
                        ...
                        print(
                            f"  - 返回值 (Response): {response_text[:300]}"
...
def convert_goofish_link(url: str) -> str:
    ...
    if match_first_link:
        ...
        return (
            f"https://pages.goofish.com/sharexy?loadingVisible=false"
            f"&bft=item&bfs=idlepc.item&spm=a21ybx.item.0.0&bfp={quote(bfp_json)}"
        )
    return url
...
async def save_to_jsonl(data_record: dict, keyword: str):
    """兼容旧调用名。这里直接 append 到 jsonl/keyword_full_data.jsonl。"""
    try:
