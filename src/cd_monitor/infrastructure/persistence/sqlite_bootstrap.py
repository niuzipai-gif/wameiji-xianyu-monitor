"""SQLite 启动初始化与旧文件迁移（适配 cd_monitor）。

职责：
- 确保 tasks / result_items / price_snapshots / app_metadata 等表存在；
- 首次启动时从 legacy config.json 把任务导入到 tasks 表；
- 首次启动时把 jsonl/*.jsonl 的历史结果导入到 result_items；
- 首次启动时把 price_history/*_history.jsonl 导入到 price_snapshots；
- 用 app_metadata 记录"已导入"标记，避免重复导入。
"""
...
from cd_monitor.infrastructure.config.settings import (
    settings, business_settings, scraper_settings,
)
from cd_monitor.infrastructure.persistence.sqlite_connection import (
    init_schema, sqlite_connection,
)
from cd_monitor.infrastructure.persistence.storage_names import (
...
log = logging.getLogger("cd_monitor.persistence")
