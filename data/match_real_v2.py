...
for _path in (_BASE_DIR.parent / "src", _BASE_DIR):
...
def fetch_watches(db_path: str) -> list[dict]:
    with sqlite3.connect(db_path) as conn:
        ...
        rows = conn.execute(
            "SELECT catalog_no, artist, title_jp, title_cn, jan, required_keywords, excluded_keywords, "
...
def _fuzzy_album_match(title_jp: str, item_title: str, threshold: float = 0.6) -> bool:
    ...
    Uses sliding-window token comparison so short album names like
    "ESCAPARADE" still match against long item titles like
    "Official髯男dism エスカパレード CD"."""
    if not title_jp or not item_title:
        return False
    import re as _re
    title_jp_clean = _re.sub(r'\s+', '', title_jp.lower())
    item_clean = _re.sub(r'\s+', '', item_title.lower())
    item_clean = item_clean.replace('ー', '-').replace('−', '-')
    # Direct substring match (cheap, runs first)
    if title_jp_clean in item_clean:
        return True
    tokens = _re.findall(r'[A-Za-z0-9぀-ヿ一-鿿]+', item_title)
    if not tokens:
        return False
    max_window = min(5, len(tokens))
    try:
        jp_romaji = _re.sub(r'\s+', '', _kks_to_romaji(title_jp))
    except Exception:
        jp_romaji = ''
    title_jp_compact = _re.sub(r'\s+', '', title_jp.lower())
    from difflib import SequenceMatcher
    for n in range(1, max_window + 1):
        for i in range(0, len(tokens) - n + 1):
            window = ''.join(tokens[i:i + n])
            window_clean = window.lower().replace('ー', '-').replace('−', '-')
            if not window_clean or len(window_clean) < 3:
                continue
            if title_jp_compact in window_clean or window_clean in title_jp_compact:
                return True
            if jp_romaji:
                try:
                    win_romaji = _re.sub(r'\s+', '', _kks_to_romaji(window))
                except Exception:
                    win_romaji = ''
                if win_romaji:
    ...
    with sqlite3.connect(db_path, timeout=60) as conn:
        ...
        for row in item_rows:
            ...
            try:
                ...
                # Normalize Japanese long vowel mark ー to ASCII hyphen for fuzzy text matching
                item_title_norm = item_title.replace("ー", "-").replace("−", "-").replace("‐", "-")
                ...
                # P1 fallback (ALWAYS run, not just when candidate_list is empty):
                # For each watch row, check if watchlist artist/title_jp/title_cn appears
                # in the item title. Add as candidate even if xianyu signals already found
                # some. This lets direct name matches win over weak signal-based candidates.
                if item_title:
                    item_compact = "".join(item_title_norm.split()).lower()
                ...
                # Track which candidates came from P1 fallback (direct artist/title match in item title)
                p1_added = set()
                if item_title:
                    item_compact_dm = "".join(item_title_norm.split()).lower()
                    for _wcat, _wrow in watch_by_catalog.items():
                        for _k in ("artist", "title_jp", "title_cn"):
                            _v = (_wrow.get(_k) or "").strip()
                            if _v and len(_v) >= 2:
                                _tv = "".join(_v.split()).lower()
                                if _tv and _tv in item_compact_dm:
                                    p1_added.add(_wcat)
                                    break
                ...
                if candidate_list and item_title:
                    ...
                    for _cand in candidate_list:
                        ...
                        if not _wr:
                            ...
                            continue
                        # P2b: bypass required_keywords check for P1-fallback candidates
                        if _cand in p1_added:
                            filtered.append(_cand)
                            continue
                ...
                # P2c: compute p1_strong set - catalogs where BOTH watchlist artist AND title_jp
                # appear (fuzzy) in item title (highest name-match signal).
                # Uses pykakasi for kana->romaji conversion to handle cross-script variations.
                p1_strong = set()
                if item_title:
                    item_compact_strong = "".join(item_title_norm.split()).lower()
                    for _wcat, _wrow in watch_by_catalog.items():
                ...
                for cat in candidate_list:
                    ...
                    # P3 bypass: if candidate was added by P1 fallback with BOTH artist AND title_jp
                    # in item title (p1_strong), allow opp creation with confidence as low as 0.10
                    # This fixes the case where katakana/romaji mismatch prevents phash matching
                    # but the item title clearly identifies the album.
                    min_conf_for_this = eval_cfg.min_match_confidence_weak or 0.4
                    if cat in p1_strong:
                        # P3: BOTH artist AND title_jp matched - high confidence name match
                        min_conf_for_this = 0.10
                    if new_conf < min_conf_for_this:
                        continue
                    ...
                    try:
                        ...
                        if _valid_samples:
                            ...
                            for _v in _valid_samples:
                                ...
                                if _ph_dist is not None:
                                    if _ph_dist <= 5:
                                        ...
                                        _score += 0.20
                                    # P1: stricter any-mode (was: phash<=12 OR title OR cover>=0.25)\n                                    # Now: phash<=14 OR (artist AND title_jp both hit) OR cover>=0.30\n                                    _score += max(0.0, (40 - _ph_dist) / 80.0)
                                # 2. artist hit
                            ...
                            for _s, _pd, _v in _sorted:
                                ...
                                # cover_jaccard>=0.30.  P1 stricter any-mode.
                                if _wmode == 'all':
                                    same_album = (
                                        _a_local_hit
                                    ...
                                    same_album = (
                                        (_ph_dist_local is not None and _ph_dist_local <= 14)
                                        or (_a_local_hit and _title_local_hit)
                                        or _cj_local >= 0.30
                                    )
                                if not same_album:
                                    continue
